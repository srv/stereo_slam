...
tf::Transform reconstruction::ReconstructionBase::readPoses(vector< pair<string, tf::Transform> > &cloud_poses)
{
  // Init
  cloud_poses.clear();
...
void reconstruction::ReconstructionBase::setParameters(string work_dir)
{
  ...
  params.clouds_dir = work_dir + "pointclouds/";
  params.graph_file = work_dir + "graph_vertices.txt";
  setParams(params);
}

tf::Transform reconstruction::ReconstructionBase::matrix4fToTf(Eigen::Matrix4f in)
{
  tf::Vector3 t_out;
